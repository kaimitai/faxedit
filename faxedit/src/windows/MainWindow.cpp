		...
		try {
			...
			if (m_emode == fe::EditMode::TilemapEditMode) {
				if (l_ctrl && ImGui::IsKeyPressed(ImGuiKey_C)) {
					if (l_shift)
						clipboard_copy_tilemap_changes(true);
					else if (l_alt)
						clipboard_copy_tilemap_changes(false);
					else
						clipboard_copy();
				}
...
void fe::MainWindow::regenerate_atlas_if_needed(SDL_Renderer* p_rnd) {
	...
	if (m_atlas_new_tileset_no != m_atlas_tileset_no ||
		...
		m_gfx.generate_atlas(p_rnd, m_cache.m_world_ppu_tilesets.at(m_atlas_new_tileset_no),
...
void fe::MainWindow::clipboard_copy_tilemap_changes(bool p_incl_header) {
...
void fe::MainWindow::load_rom(SDL_Renderer* p_rnd, const std::string& p_filepath,
	...
	try {
		...
		// build anything that can throw before committing
		auto nes_palette{ fe::game::gfx::parse_nes_palette(loaded.config) };

		// everything succeeded - commit
		m_config = std::move(loaded.config);
		m_game = std::move(loaded.game);
		m_region_override = p_region;
		m_cache.m_nes_palette = std::move(nes_palette);
