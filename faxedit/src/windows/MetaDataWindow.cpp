...
void fe::MainWindow::draw_metadata_window(SDL_Renderer* p_rnd) {
	...
	if (ImGui::BeginTabBar("Metadata")) {
		...
		if (ImGui::BeginTabItem(std::format("World {} ({}) Metadata###wmdtabi", m_sel_chunk,
			...
			if (ImGui::BeginTabBar("gamemetatabs")) {
				...
				if (ImGui::BeginTabItem("Push-Block")) {
					...
					ui::imgui_slider_with_arrows("###pbstage",
						std::format("Stage {} ({})",
							l_pb.m_stage, m_cache.m_labels_worlds[l_world]),
						l_pb.m_stage, 0, 5, "");
				...
				if (!m_cache.m_disable_pal2_mus && ImGui::BeginTabItem("Pal2Mus")) {
					...
					if (!slots_empty) {
						...
						ui::imgui_slider_with_arrows("###p2mp",
							std::format("Palette: {}",
								get_description(slot.m_palette, m_cache.m_labels_palettes)),
							slot.m_palette, 0, m_game->m_palettes.size() - 1);

						ui::imgui_slider_with_arrows("###p2mm",
							std::format("Music: {}",
								get_description(slot.m_music, m_cache.m_labels_music)),
							slot.m_music, 0, m_cache.m_music_count);
				...
				if (ImGui::BeginTabItem("Fog")) {
					...
					ui::imgui_slider_with_arrows("###fogw",
						std::format("World: {}",
							get_description(l_fog.m_world_no, m_cache.m_labels_worlds)),
						l_fog.m_world_no, 0, m_game->m_chunks.size()
					);

					ui::imgui_slider_with_arrows("###fogp",
						std::format("Palette: {}",
							get_description(l_fog.m_palette_no, m_cache.m_labels_palettes)),
...
void fe::MainWindow::show_stages_data(void) {
	...
	ui::imgui_slider_with_arrows("###stens",
		std::format("Next Stage: {} ({})",
			...
			m_cache.m_labels_worlds[l_stages.m_stages[l_stage.m_next_stage].m_world_id]),
	...
	ui::imgui_slider_with_arrows("###stenr", std::format("Next-Stage Door Requirement: {}",
		get_description(l_stage.m_next_requirement, m_cache.m_labels_door_reqs)),
		l_stage.m_next_requirement, 0, m_cache.m_labels_door_reqs.size() - 1);
	...
	ui::imgui_slider_with_arrows("###steps",
		std::format("Previous Stage: {} ({})",
			...
			m_cache.m_labels_worlds[l_stages.m_stages[l_stage.m_prev_stage].m_world_id]),
	...
	ui::imgui_slider_with_arrows("###stepr", std::format("Previous-Stage Door Requirement: {}",
		get_description(l_stage.m_prev_requirement, m_cache.m_labels_door_reqs)),
		l_stage.m_prev_requirement, 0, m_cache.m_labels_door_reqs.size() - 1);
	...
	if (ImGui::BeginTabItem("Metatiles")) {
		...
		ImGui::SeparatorText(std::format("Block property: {}",
			get_description(l_mt_def.m_block_property,
				m_cache.m_labels_block_props)).c_str());
...
void fe::MainWindow::show_scene(fe::Scene& p_scene, bool p_show_pos) {
	...
	if (fe::ui::imgui_slider_with_arrows("##wsp",
		std::format("Palette: {}",
			get_description(static_cast<byte>(p_scene.m_palette), m_cache.m_labels_palettes)),
		p_scene.m_palette, 0,
	...
	if (fe::ui::imgui_slider_with_arrows("###wst",
		std::format("Tileset: {}", get_description(static_cast<byte>(p_scene.m_tileset),
	...
	fe::ui::imgui_slider_with_arrows("###wsm",
		std::format("Music: {}", get_description(static_cast<byte>(p_scene.m_music),
			m_cache.m_labels_music)),
		p_scene.m_music, 0, m_cache.m_music_count);
