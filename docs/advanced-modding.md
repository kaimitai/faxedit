...
| AtlasDevSwapVar | Byte, Byte | Swaps two script registers; an invalid register on either side does nothing | AtlasDevSwapVar 0 3 ; registers 0 and 3 exchange values |
| AtlasDevIfRandomChance | Byte, Label | Jumps when the shared generator AtlasDevRandomVar uses draws a byte below Probability; 0 never branches and does not step the generator | AtlasDevIfRandomChance 32 @maybe ; roughly one call in eight |
...
| AtlasDevSetExperience | Short (0-65,535) | Replaces experience exactly, recomputes current and pending title 0-15 through the vanilla threshold checker, and redraws the experience HUD | AtlasDevSetExperience 3000 |
...
Event 0 checks the Mascon-path quest bit once. It cancels itself immediately when the bit is clear; when set, it runs the hardcoded path change and then cancels itself. Event 1 manages boss music while at least one class-7 entity exists, then restores area music. Event 2 starts the ending when all eight entity slots are empty.

These handlers keep their original assumptions. Event 1 does not spawn a boss, lock the room or create a reward. Events 0 and 2 should only be used in their intended story context.

```AtlasDevSpawnEntity``` reruns the full entity CHR pass starting at PPU CHR address ```$0900```. Portrait graphics use the same CHR area, so spawning while a portrait is displayed replaces its tiles with entity graphics. Use a generic textbox or clear the portrait first.
