...
// yuinaru control: tunes yuinaru's two swoops. sideways and up and down, its
...
// nothing until a value is set. it still flies through walls and turns at the
// screen edges, through the game's own movers.
//
// one bank 14 instruction is retargeted: the first load of its movement, at
// $8ff9, reached after its setup. the hook runs the same movement with the
...
namespace {
	...
	constexpr word TRI{ 0x83e1 }, SHIFT_X{ 0x83c1 }, SHIFT_Y{ 0x83d1 }, MOVE_X{ 0x8427 }, MOVE_Y{ 0x859c };
	constexpr word FLAG_BASE{ 0x0101 }, SITE_INIT{ 0x8fe7 }, SITE{ 0x8ff9 }, V_CONT{ 0x8ffc }, HELPERS{ 0x83c1 };
	...
	constexpr word SITE_Y{ 0x9017 }, X_SCALE{ 0x9003 }, X_SCALE_OP{ 0x9004 }, X_MOVER{ 0x900e };
	constexpr word Y_SCALE{ 0x9021 }, Y_SCALE_OP{ 0x9022 }, Y_MOVER{ 0x902c };
	constexpr int STOCK_CAP_X{ 3 }, STOCK_CAP_Y{ 1 };

	constexpr std::array<byte, 18> INIT_ORIG{ 0x20, 0x85, 0xa8, 0xd0, 0x0d, 0xa9, 0x00, 0x9d, 0xec, 0x02, 0xa9, 0x80, 0x9d, 0xf4, 0x02, 0x20, 0x94, 0xa8 };
	constexpr std::array<byte, 3> SITE_ORIG{ 0xbd, 0xec, 0x02 };
	constexpr std::array<byte, 55> BODY_ORIG{
		0xa0, 0x01, 0x20, 0xe1, 0x83, 0xa0, 0x03, 0x20, 0xc1, 0x83, 0xad, 0x75, 0x03, 0x29, 0x03, 0x8d,
	...
	constexpr std::array<byte, 36> MOVE_X_ORIG{
		0x20, 0x9a, 0x84, 0xb5, 0xba, 0xc9, 0xf0, 0x90, 0x19, 0xbd, 0xdc, 0x02, 0x4a, 0xb5, 0xba, 0x29,
		0xf0, 0xb0, 0x03, 0x18, 0x69, 0x10, 0x95, 0xba, 0xbd, 0xdc, 0x02, 0x49, 0x01, 0x9d, 0xdc, 0x02,
		0x38, 0x60, 0x18, 0x60 };
	constexpr std::array<byte, 40> MOVE_Y_ORIG{
		0x20, 0xca, 0x85, 0x20, 0xad, 0x85, 0x90, 0x08, 0xbd, 0xdc, 0x02, 0x49, 0x80, 0x9d, 0xdc, 0x02,
...
word fh::HackManager::install_AtlasDevYuinaruControl(const fe::Config&, std::vector<byte>& p_rom, word cpu_addr,
	const fh::GeneralHack& p_hack) const {
	const std::string name{ "AtlasDevYuinaruControl" };
	...
	const int xspeed{ get("xspeed", 32) }, xloop{ get("xloop", 256) }, yspeed{ get("yspeed", 16) }, yloop{ get("yloop", 128) };
	...
	const bool x_changed{ xspeed != 32 || xloop != 256 }, y_changed{ yspeed != 16 || yloop != 128 };
	...
	if (flag < 0) {
		...
		at(0x8ffd) = static_cast<byte>(kx); if (sx > 0) at(0x9002) = static_cast<byte>(sx);
		at(0x900a) = static_cast<byte>(cap_x);
		at(0x901b) = static_cast<byte>(ky); if (sy > 0) at(0x9020) = static_cast<byte>(sy);
		at(0x9028) = static_cast<byte>(cap_y);
		if (size == 0)
			return cpu_addr;
