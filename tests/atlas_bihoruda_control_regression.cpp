...
// pins install_AtlasDevBihorudaControl to known bytes for the default, a half
// pixel sideways peak, the fastest and shortest swoops, a half pixel up and
// down peak and a flag, and checks that it installs next to yuinaru control
namespace {
	constexpr word ORG{ 0xbdb5 };
	constexpr std::size_t ROM_SIZE{ 0x40010 }, YUINARU_SIZE{ 9 };
	const std::vector<byte> INIT_ORIG{ 0x20, 0x85, 0xa8, 0xd0, 0x0d, 0xa9, 0x00, 0x9d, 0xec, 0x02, 0xa9, 0x40, 0x9d, 0xf4, 0x02, 0x20, 0x94, 0xa8 };
	const std::vector<byte> SITE_ORIG{ 0xbd, 0xec, 0x02 };
	const std::vector<byte> BODY_ORIG{ 0xa0, 0x02, 0x20, 0xe1, 0x83, 0xa0, 0x03, 0x20, 0xc1, 0x83, 0xad, 0x75, 0x03, 0x29, 0x01, 0x8d,
	...
	// yuinaru control's sites, for the pair
	const std::vector<byte> YU_INIT_ORIG{ 0x20, 0x85, 0xa8, 0xd0, 0x0d, 0xa9, 0x00, 0x9d, 0xec, 0x02, 0xa9, 0x80, 0x9d, 0xf4, 0x02, 0x20, 0x94, 0xa8 };
	const std::vector<byte> YU_BODY_ORIG{ 0xa0, 0x01, 0x20, 0xe1, 0x83, 0xa0, 0x03, 0x20, 0xc1, 0x83, 0xad, 0x75, 0x03, 0x29, 0x03, 0x8d,
	...
	const std::vector<byte> YU_MOVE_X_ORIG{ 0x20, 0x9a, 0x84, 0xb5, 0xba, 0xc9, 0xf0, 0x90, 0x19, 0xbd, 0xdc, 0x02, 0x4a, 0xb5, 0xba, 0x29,
		0xf0, 0xb0, 0x03, 0x18, 0x69, 0x10, 0x95, 0xba, 0xbd, 0xdc, 0x02, 0x49, 0x01, 0x9d, 0xdc, 0x02,
		0x38, 0x60, 0x18, 0x60 };
	const std::vector<byte> YU_MOVE_Y_ORIG{ 0x20, 0xca, 0x85, 0x20, 0xad, 0x85, 0x90, 0x08, 0xbd, 0xdc, 0x02, 0x49, 0x80, 0x9d, 0xdc, 0x02,
		0x60, 0xb5, 0xc2, 0xc9, 0xe0, 0x90, 0x10, 0xbd, 0xdc, 0x02, 0x0a, 0xb5, 0xc2, 0x29, 0xf0, 0xb0,
		0x03, 0x18, 0x69, 0x10, 0x95, 0xc2, 0x38, 0x60 };

	// bihoruda with a half-pixel peak on a 256-frame loop (which needs the rewritten movement), placed after
	// yuinaru with the same
	...
	const std::vector<Shape> SHAPES{
		{ "AtlasDevBihorudaControl", 0, {
		} },
		{ "AtlasDevBihorudaControl xspeed=4 xloop=256", 9, {
			{ 14, 0x8ee5, "01" },
			{ 14, 0x8eec, "b5bd" },
			{ 14, 0x8ef2, "00" },
			{ 14, 0xbdb5, "8d7403a9008d750360" },
		} },
		{ "AtlasDevBihorudaControl xspeed=64 xloop=8 yspeed=64 yloop=8", 0, {
			{ 14, 0x8ee5, "06" },
			{ 14, 0x8eea, "09" },
			{ 14, 0x8ef2, "07" },
			{ 14, 0x8f00, "06" },
		...
		{ "AtlasDevBihorudaControl flag=12", 0, {
		} },
		{ "AtlasDevBihorudaControl xspeed=64 xloop=8 yspeed=64 yloop=8 flag=12", 64, {
			{ 14, 0x8ee1, "4cb5bd" },
			{ 14, 0xbdb5, "ad02012910d006bdec024ce48ebdec02a00620e183a00920c183ad750329078d7503201984feec02bdf402a00620e183a00920d183ad770329078d77034c118f" },
		} },
		{ "AtlasDevBihorudaControl xspeed=64 flag=12", 37, {
			{ 14, 0x8ee1, "4cb5bd" },
			{ 14, 0xbdb5, "ad02012910d006bdec024ce48ebdec02a00220e183a00520c183ad750329078d75034cf68e" },
		} },
		{ "AtlasDevBihorudaControl yspeed=4 yloop=256 flag=12", 40, {
			{ 14, 0x8efc, "4cb5bd" },
			{ 14, 0xbdb5, "ad02012910d006bdf4024c" },
			{ 14, 0xbdc1, "8ebdf402a00120e1838d7603a9008d7703ad770329008d77034c118f" },
		} },
		{ "AtlasDevBihorudaControl mode=vanilla", 0, {
		} },
	};
	...
	std::vector<byte> vanilla_rom() {
		...
		put(rom, 0x8ecf, INIT_ORIG); put(rom, 0x8ee1, SITE_ORIG); put(rom, 0x8ee4, BODY_ORIG);
		put(rom, 0x83c1, HELPERS_ORIG); put(rom, 0x8419, MOVE_X_ORIG); put(rom, 0x8564, MOVE_Y_ORIG);
		put(rom, 0x8fe7, YU_INIT_ORIG); put(rom, 0x8ff9, SITE_ORIG); put(rom, 0x8ffc, YU_BODY_ORIG);
		put(rom, 0x8427, YU_MOVE_X_ORIG); put(rom, 0x859c, YU_MOVE_Y_ORIG);
		return rom;
	}
	...
	void require_untouched(const std::vector<byte>& rom, const std::string& spec) {
		require(hex_at(rom, 0x8ecf, INIT_ORIG.size()) == hex(INIT_ORIG), spec + ": the setup must stay");
...
int main() {
	try {
		...
		{
			auto rom{ vanilla_rom() };
			const std::string spec{ "AtlasDevYuinaruControl xspeed=4\nAtlasDevBihorudaControl xspeed=4 xloop=256" };
			const auto n{ install(rom, spec) };
			require(n == YUINARU_SIZE + PAIR_CODE.size() / 2, "the pair: size " + std::to_string(n));
			require(hex_at(rom, 0x9004, 2) == "b5bd", "the pair: yuinaru's scaler call " + hex_at(rom, 0x9004, 2));
			require(hex_at(rom, 0x8eec, 2) == "bebd", "the pair: bihoruda's scaler call " + hex_at(rom, 0x8eec, 2));
			require(hex_at(rom, 0xbdbe, PAIR_CODE.size() / 2) == PAIR_CODE, "the pair: bihoruda's code differs");
			require_untouched(rom, spec);
		}
		{
			const auto pristine{ vanilla_rom() };
			auto rom{ vanilla_rom() };
			require(install(rom, "AtlasDevBihorudaControl mode=vanilla") == 0, "vanilla installs nothing");
			require(rom == pristine, "vanilla leaves the rom byte identical");
		}
		{
			const auto pristine{ vanilla_rom() };
			for (const char* bad : { "AtlasDevBihorudaControl xspeed=24", "AtlasDevBihorudaControl yspeed=128",
				"AtlasDevBihorudaControl xloop=512", "AtlasDevBihorudaControl yloop=4", "AtlasDevBihorudaControl flag=248",
				"AtlasDevBihorudaControl mode=on", "AtlasDevBihorudaControl speed=3",
				"AtlasDevBihorudaControl mode=vanilla xloop=3" }) {
				auto rom{ vanilla_rom() };
				bool threw{ false };
		...
		}
		{
			...
			bool threw{ false };
			try { install(rom, "AtlasDevBihorudaControl"); } catch (const std::exception&) { threw = true; }
			require(threw && rom == before, "a non vanilla wave routine is refused and nothing is written");
		}
		{
			auto rom{ vanilla_rom() };
			put(rom, 0x8ee1, { 0x4c, 0x00, 0x90 });   // something else owns the site
			bool threw{ false };
			try { install(rom, "AtlasDevBihorudaControl"); } catch (const std::exception&) { threw = true; }
			require(threw, "a site patched by something else is refused");
		}
		std::cout << "atlas_bihoruda_control_regression: ok\n";
	}
	catch (const std::exception& e) { std::cerr << "atlas_bihoruda_control_regression: " << e.what() << "\n"; return 1; }
	return 0;
}
